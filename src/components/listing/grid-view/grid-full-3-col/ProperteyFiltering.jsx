    ...
    useEffect(() => {
        ...
        const refItems = listings.filter((elm) => {
          ...
          if (priceRange.length > 0) {
            const filtered = refItems.filter((elm) => {
              // Fiyatın doğru formatta olup olmadığını kontrol et
              if (!elm.price || !(elm.price.includes('$') || elm.price.includes('₺'))) {
                return false;
              }
          
              // Fiyatı ayır ve sayıya dönüştür
              const priceValue = elm.price.includes('$')
